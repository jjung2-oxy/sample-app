...
import { SuggestionPulldowns } from '../../components/SuggestionPulldowns';
...
const Chat = () => {
  ...
  const handleSuggestionSelected = (item: SuggestionItem) => {
    setSelectedSuggestion(item);
    setInputValue(item.question || '');
  };

  const handleSendMessage = (question: ChatMessage["content"]) => {
    let fullPrompt: string;
    if (typeof question === 'string') {
      fullPrompt = selectedSuggestion 
        ? `${selectedSuggestion.context} ${question}`
        : question;
    } else {
      fullPrompt = selectedSuggestion?.context ?? '';
      fullPrompt += question[0].text;
    }
    
    if (appStateContext?.state.currentChat?.id) {
      appStateContext?.state.isCosmosDBAvailable?.cosmosDB
        ? makeApiRequestWithCosmosDB(fullPrompt, appStateContext.state.currentChat.id)
        : makeApiRequestWithoutCosmosDB(fullPrompt, appStateContext.state.currentChat.id);
    } else {
      appStateContext?.state.isCosmosDBAvailable?.cosmosDB
        ? makeApiRequestWithCosmosDB(fullPrompt)
        : makeApiRequestWithoutCosmosDB(fullPrompt);
    }
        ...
        <Stack horizontal className={styles.chatRoot}>
          <div className={styles.chatContainer}>
            ...
            <Stack horizontal className={styles.chatInput}>
              ...
              <Stack.Item grow={1} className={styles.inputContainer}>
                <SuggestionPulldowns onSuggestionSelected={handleSuggestionSelected} />
                <QuestionInput
                  clearOnSend
